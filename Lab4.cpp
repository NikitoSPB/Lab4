...
{
    ifstream File("C:\\cpp\\textfile.txt");
    if (!File.is_open())
    {
        cout << "File not open!\n";
        exit(1);
...
void f_punktMarks(string* pmS1)
{
    ...
    for (int i = 0; ((*pmS1)[i] == '.') || ((*pmS1)[i] == ',') || ((*pmS1)[i] == ';') || ((*pmS1)[i] == ':') || ((*pmS1)[i] == '!') || ((*pmS1)[i] == '?'); i++)
        countPunktMarks++;
    (*pmS1).erase(0, countPunktMarks);
    int lenSpS1 = (*pmS1).length();
    for (int i = 0; i < lenSpS1; i++)
    {
        if (((*pmS1)[i] == '.') || ((*pmS1)[i] == ',') || ((*pmS1)[i] == ';') || ((*pmS1)[i] == ':') || ((*pmS1)[i] == '!') || ((*pmS1)[i] == '?'))
        {
            char mark = (*pmS1)[i];
            countMarks = 0;
            while (((*pmS1)[i + countMarks + 1] == '.') || ((*pmS1)[i + countMarks + 1] == ',') || ((*pmS1)[i + countMarks + 1] == ':') || ((*pmS1)[i + countMarks + 1] == '?') || ((*pmS1)[i + countMarks + 1] == '!') || ((*pmS1)[i + countMarks + 1] == ';'))
                countMarks++;
            countPunktMarks = 0;
